const { CreateGroupChatModel } = require('../../models/conversation.model')
...
const AddMemberToGroupChat = async (req, res) => {
  ...
  for (let i = 0; i < _checkExistMember.length; i++) {
    if (_checkId[i] == true) {
      ...
      return res.status(200).json({ message: 'add member to group success' })
    }
    else {
...
const CreateIndivisualConversations = async () => {
